...
export function realpathOr(path) {
    try {
...
const isActive = (s) => s.status !== "approved";
export function resolveSession(sessions, explicit, cwd) {
    ...
    if (pointer !== undefined) {
        ...
        if (!session) {
            ...
            fail("E_STALE_POINTER", `${currentSessionPath(root)} points at ${pointer}, which is not in the daemon registry; run otacon start or pass --session <id>`);
        }
        if (!isActive(session)) {
            ...
            fail("E_SESSION_OVER", `${currentSessionPath(root)} points at ${pointer}, which is ${session.status} — that session is over; run otacon start for the next plan or pass --session <id> explicitly`);
        }
        return session;
    }
    ...
    if (here.length === 1)
        return here[0];
    if (here.length === 0) {
        fail("E_NO_SESSION", `no active otacon session for ${root}; run otacon start (or pass --session <id>)`);
    }
    fail("E_AMBIGUOUS_SESSION", `${here.length} active sessions for ${root} and no .otacon/current-session pointer; pass --session <id>`, { sessions: here.map((s) => ({ id: s.id, title: s.title, status: s.status })) });
}
