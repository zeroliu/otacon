 ...
 */
export function segmentPlan(content) {
...
export function diffLines(a, b) {
    ...
    const out = a.slice(0, head).map((text) => ({ op: "context", text }));
    let i = 0;
    let j = 0;
    while (i < am.length && j < bm.length) {
        if (am[i] === bm[j]) {
            out.push({ op: "context", text: am[i] });
            i++;
            j++;
        }
        else if (lcs[i + 1][j] >= lcs[i][j + 1]) {
            out.push({ op: "del", text: am[i] });
            i++;
        }
        else {
            out.push({ op: "add", text: bm[j] });
            j++;
        }
    }
    for (; i < am.length; i++)
...
export function buildHunks(diff, context = 3) {
    ...
    return ranges.map(([rawStart, rawEnd]) => {
        ...
        return {
            fromStart: fromLine[start],
            fromCount: lines.filter((l) => l.op !== "add").length,
            toStart: toLine[start],
 ...
 */
export function diffPlans(fromContent, toContent) {
