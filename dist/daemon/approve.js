 ...
 */
export function pickArtifactRelPath(repo, title, date) {
    const slug = slugify(title) || "plan";
    for (let n = 1;; n++) {
        const name = n === 1 ? `${date}-${slug}.md` : `${date}-${slug}-${n}.md`;
        if (!existsSync(join(plansDir(repo), name)))
            return join("docs", "plans", name);
    }
}
...
function renderEntry(entry) {
    ...
    if (entry.options) {
        const labeled = entry.options.map((o) => o === entry.recommend ? `${o} (recommended)` : o);
 ...
 */
export function composeArtifact(markdown, opts) {
    ...
    for (let i = 1; i < close; i++) {
        ...
        else if (/^revision:/.test(lines[i] ?? ""))
            lines[i] = `revision: ${opts.revision}`;
    }
    let out = lines.join("\n");
