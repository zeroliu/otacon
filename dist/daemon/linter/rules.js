...
export function checkL1(plan, session) {
    ...
    for (const section of plan.sections) {
        if (!requiredIds.includes(section.id)) {
            issues.push(issue("L1", "E_UNKNOWN_SECTION", "error", `Unknown section "## ${section.title}" — the schema allows only Summary, Decisions, Phases, Risks, Open Questions`, { line: section.startLine, section: section.id }));
        }
