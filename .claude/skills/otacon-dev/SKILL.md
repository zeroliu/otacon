...
3. **Finish — write a reviewer-first PR body.** Author the body by PORTING the approved
   ...
   default**: run `./bin/otacon config get pr.draft` and pass `--draft` to `gh pr create`
   ...
   the session; `./bin/otacon status` reports `prUrl`); the amendment does NOT change the PR's
