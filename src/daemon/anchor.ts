...
export type RelocateResult = { state: "anchored"; anchor: Anchor } | { state: "orphaned" };
...
interface Normalized {
  text: string;
  /** map[i] = raw index of normalized char i. */
  map: number[];
}
...
interface Candidate {
  unit: PlanUnit;
  /** Raw [start, end) span of the match within the unit's text. */
  start: number;
  end: number;
}
...
function contextScore(anchor: Anchor, text: string, candidate: Candidate): number {
...
function disambiguate(
  anchor: Anchor,
  candidates: { candidate: Candidate; text: string }[],
): Candidate | undefined {
  ...
  if (best && (!second || best.score > second.score)) return best.candidate;
...
export function relocateAnchor(
  ...
  const raw: { candidate: Candidate; text: string }[] = [];
  ...
  if (needle === "") return { state: "orphaned" };
  const fuzzy: { candidate: Candidate; text: string; norm: Normalized }[] = [];
  for (const unit of units) {
    const text = unit.lines.join("\n");
    const norm = normalize(text);
    for (const idx of findAll(norm.text, needle)) {
      const start = norm.map[idx] as number;
      const end = (norm.map[idx + needle.length - 1] as number) + 1;
      fuzzy.push({ candidate: { unit, start, end }, text, norm });
    }
  }
  if (fuzzy.length === 0) return { state: "orphaned" };
  const match = disambiguate(anchor, fuzzy);
  if (!match) return { state: "orphaned" };
