...
import type { GrillAnswer, TranscriptEntry, TranscriptFile } from "../shared/types.js";
import { quarantineCorruptFile, readJsonOr, stringify, writeFileAtomic } from "./store.js";

function isAnswer(raw: unknown): raw is GrillAnswer {
  const answer = raw as GrillAnswer;
  if (typeof answer !== "object" || answer === null) return false;
  if (typeof answer.answeredAt !== "string") return false;
  if (answer.choice !== undefined && typeof answer.choice !== "string") return false;
  if (
...
function isEntry(raw: unknown): raw is TranscriptEntry {
  ...
  if (entry.recommend !== undefined && typeof entry.recommend !== "string") return false;
  if (entry.multi !== undefined && typeof entry.multi !== "boolean") return false;
  return entry.answer === undefined || isAnswer(entry.answer);
}

function parseTranscript(raw: unknown): TranscriptFile | undefined {
  const file = raw as TranscriptFile;
  const valid =
    typeof file === "object" &&
    file !== null &&
    file.version === 1 &&
...
export function answerEntry(
  ...
  entry.answer = answer;
  writeFileAtomic(path, stringify({ version: 1, entries } satisfies TranscriptFile));
  return { ...entry };
}
