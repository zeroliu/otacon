...
| Rule | Check                                                                                                                                            | Severity                               |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------- |
| L1   | Schema completeness: required sections present, in canonical order (absent optionals tolerated); phases have Goal/Files/Verification; a Files **table** needs a "What changed" column (≥2 columns) with every body row's cell filled (`E_FILES_NO_SUMMARY`), while a Files **list** is accepted as-is and `E_FILES_EMPTY` fires only when Files has neither a table nor a list; `gwt` blocks well-formed and under Verification | error                                  |
