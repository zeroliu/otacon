...
Decisions are grouped under the fixed subsystem `##` sections listed below; within a
section the **newest** decision sits at the **top**. When a decision is superseded it is
**deleted**, not kept — its still-true rationale is folded into the surviving decision's
**Why**, so two entries never disagree. To find the live decision on a subsystem, open its
section and read down from the top.
...
- [Plan schema & linter](#plan-schema--linter)
- [Protocol & events](#protocol--events)
- [Review loop, diff & comments](#review-loop-diff--comments)
- [Grill & interview](#grill--interview)
- [Review UI & rendering](#review-ui--rendering)
- [Session nav & app shell](#session-nav--app-shell)
- [Live activity & adapters](#live-activity--adapters)
- [Implement & build](#implement--build)
- [Notifications](#notifications)
- [Install & distribution](#install--distribution)
- [Project & docs process](#project--docs-process)
...
- **Decision:** `otacon start` no longer reads, writes, or migrates the repo's
  ...
  sessions store) instead of the repo-relative `.otacon/worktrees` (which "Build layout: one commit per green phase, PR vs default branch"
  set); the config *layering* and the per-phase-commit build are unchanged.
...
- **Decision:** Every session is deletable from the review UI (index card + session
  ...
  value. **Approved** → the clean path in "clean: daemon deregisters AND archives; undrained events leave with the dir" (deregister + `archiveSessionDir`,
...
- **Decision:** `DELETE /api/sessions/:id` is **status-branched**; its **approved**
  ...
  branch (see "delete a session from the UI: any status; approved archives, pending hard-removes").
...
- **Revisit when:** clean itself should bulk-sweep pending sessions (today the UI deletes
  those one at a time — see "delete a session from the UI: any status; approved archives, pending hard-removes"), which would need a real force/drain story.
...
- **Decision:** One new `CONFIG_SCHEMA` path leaf `plans.dir` (default `.otacon/plans`,
  ...
  and leaves tracking to the user (it manages no `.gitignore`, see "otacon manages no `.gitignore`; build worktrees live in `~/.otacon`"), so the default
...
- **Decision:** One `CONFIG_SCHEMA` (field metadata: section/key/label/type/default/min)
  ...
  a `worktree.dir` field (`type:"path"`, default `~/.otacon/worktrees` — see
  "otacon manages no `.gitignore`; build worktrees live in `~/.otacon`"; was `.otacon/worktrees` when first added).
...
- **Decision:** Project config goes two-tier, mirroring Claude Code's `settings.json`
  ...
  **gitignored, personal** override. Precedence is defaults ← user
  ...
  manages no `.gitignore`; build worktrees live in `~/.otacon`"; whether `config.local.json` is tracked is the user's
  call.)
...
- **Why:** Under normal operation "every open thread" is the self-healing scope — each
  ...
  *is* that verb; see "Reviewer-driven resolution: agent replies = response, reviewer Resolve = close".)
...
- **Revisit when:** `otacon clean` archives sessions while the daemon runs —
  ...
  is safe — see "clean: daemon deregisters AND archives; undrained events leave with the dir".)*
...
- **Why:** Re-rendering the memo'd `PlanView` to paint would rewrite the dossier DOM (see
  "The plan renderer is memo'd: a re-render rewrites the DOM and kills selections") — collapsing an in-progress selection and re-running mermaid. The
...
- **Revisit when:** The collapse state should sync across devices (it would move to daemon
  config). The remembered-width follow-up is now its own decision, "Drag-resizable
...
- **Decision:** The expanded live console now defaults to collapsed and never auto-expands.
  The original behavior ("Live activity is an always-on now-playing bar, not a buried collapsible fold") opened the console automatically on `draft`/`implementing`
...
- **Why:** With otacon never committing the plan (see "Approve = Save vs Implement; otacon never git-commits a plan") and the
...
- **Decision:** The build runs in a git worktree at `<worktree.dir>/<slug>` (default
  `~/.otacon/worktrees`, see "otacon manages no `.gitignore`; build worktrees live in `~/.otacon`") on branch
...
> it, keyed on the build worktree": terminal states are no longer strictly
...
- **Revisit when:** The release wants a non-default base branch. (Prerelease dist-tag
  publishing is now wired, see "Staging channel: `staging` dist-tag, suffix-routed inside the one release workflow".)
...
- **Why:** Verified conventions (June 2026): all three agents now read the
  ...
  recorded separately in "Codex moves to a `.codex/skills/` SKILL.md folder"). OpenCode also reads `~/.claude/skills/`, so the Claude
